...
TEMPLATE = (
    ...
    " var tb=document.getElementById('tb');tb.innerHTML='';\n var cur='',shown=0,cnt={};\n D.forEach(function(r){\n  if(LOCS.length&&!LOCON[r[6]])return;      // location switched off\n  var sel=SEL[r[10]];\n  if(sel.indexOf(r[4])<0)return;\n  // A class you have BOOKED is never hidden by the convenience\n  // toggles. 'Bookable now only' means 'nothing I cannot take' - and\n  // a booked class is not bookable, so it vanished from the schedule,\n  // which reads as 'my bookings are missing'. Unticking a class type\n  // still hides it: that one is a deliberate choice about content.\n  var mine=r[7]==='booked';\n  if(!mine){\n   if(av.checked&&r[7]!=='bookable')return;\n   if(hf.checked&&r[7]==='full')return;\n   if(hw.checked&&r[9]<5&&r[1]>=480&&r[1]<=1035)return;\n  }\n  cnt[r[7]]=(cnt[r[7]]||0)+1;shown++;\n  cur=addRow(tb,r,cur);\n });\n document.getElementById('none').style.display=shown?'none':'block';\n"
    ' document.getElementById(\'shown\').textContent=shown+\' of \'+D.length+\' shown\';\n var ms=document.getElementById(\'mshown\');if(ms)ms.textContent=shown+\' of \'+D.length+\' shown\';\n var lab=[[\'bookable\',\'Bookable\'],[\'full\',\'Full\'],[\'soon\',\'Not yet open\'],[\'booked\',\'Booked\'],[\'closed\',\'Closed\'],[\'unknown\',\'Status unknown\'],[\'open\',\'In the timetable\']];\n var lg=document.getElementById(\'lg\');lg.innerHTML=\'\';\n lab.forEach(function(p){\n  if(!cnt[p[0]])return;\n  var e=document.createElement(\'span\');\n  e.innerHTML=\'<i style="background:\'+SC[p[0]]+\'"></i>\'+p[1]+\' <b>\'+cnt[p[0]]+\'</b>\';\n  lg.appendChild(e);\n });\n}\n// Favouriting from the schedule is the same act as favouriting a filter\n// chip: one list, one storage key. Delegated from the table body because\n// draw() replaces every row.\ndocument.getElementById(\'tb\').addEventListener(\'click\',function(e){\n'
    " var b=e.target&&e.target.closest?e.target.closest('.rstar'):null;\n if(!b)return;\n var fk=b.getAttribute('data-fk'), i=favs.indexOf(fk);\n if(i>-1)favs.splice(i,1);else favs.push(fk);\n s(LS.v,favs);\n place(fk.split('|')[0]);          // move the chip into or out of the star row\n draw();                            // repaint the stars in the table\n});\nrefreshAll();\ndraw();\n// The filters were sticky, so they held the top of the window all the way down\n// a 2000-row schedule. Let them scroll away like anything else, and leave a\n// slim strip in their place that expands over the page when you need them.\n(function(){\n var bar=document.getElementById('filters'),mb=document.getElementById('minibar'),\n     mbt=document.getElementById('mbt'),mbc=document.getElementById('mbc'),\n     fs=document.getElementById('fscrim'),mq=window.matchMedia('(max-width:640px)');\n if(!bar||!mb)return;\n"
    " function open(){return bar.classList.contains('float')}\n function float(on){\n  bar.classList.toggle('float',on);\n  bar.style.top=on?(mb.offsetHeight+'px'):'';\n  fs.classList.toggle('on',on);\n  mbt.setAttribute('aria-expanded',on?'true':'false');\n  mbc.textContent=on?'\\u25b4':'\\u25be';\n  if(on)mb.classList.add('on');\n }\n function update(){\n  if(mq.matches){float(false);mb.classList.remove('on');return;}   // phones use the hamburger\n  if(open()){mb.classList.add('on');return;}\n  mb.classList.toggle('on', bar.getBoundingClientRect().bottom<=0);\n }\n mbt.addEventListener('click',function(){float(!open())});\n fs.addEventListener('click',function(){float(false)});\n document.addEventListener('keydown',function(e){if(e.key==='Escape'&&open())float(false)});\n window.addEventListener('scroll',update,{passive:true});\n window.addEventListener('resize',update);\n update();\n})();\n"
    "// Mobile: one hamburger holds everything that is not the schedule itself.\n// The controls are MOVED, not duplicated - a second copy of a checkbox is a\n// second source of truth, and they drift. Every listener above still applies\n// because these are the same nodes.\n(function(){\n var mq=window.matchMedia('(max-width:640px)');\n var drawer=document.getElementById('drawer'),scrim=document.getElementById('scrim'),\n     dbody=document.getElementById('dbody'),btn=document.getElementById('mnu'),\n     dclose=document.getElementById('dclose');\n if(!drawer||!btn)return;\n var blocks=['.sub','#lg','#booked','.bar'].map(function(sel){\n  var el=document.querySelector(sel);\n  return el?{el:el,parent:el.parentNode,next:el.nextSibling}:null;\n }).filter(Boolean);\n var inMenu=false;\n function toMenu(){if(inMenu)return;blocks.forEach(function(b){dbody.appendChild(b.el)});inMenu=true;}\n"
    " function toPage(){if(!inMenu)return;blocks.forEach(function(b){b.parent.insertBefore(b.el,b.next)});inMenu=false;}\n function show(on){\n  drawer.classList.toggle('open',on);scrim.classList.toggle('on',on);\n  btn.setAttribute('aria-expanded',on?'true':'false');\n  document.body.classList.toggle('locked',on);\n }\n function apply(){ if(mq.matches){toMenu();} else {show(false);toPage();} }\n btn.addEventListener('click',function(){show(!drawer.classList.contains('open'))});\n scrim.addEventListener('click',function(){show(false)});\n dclose.addEventListener('click',function(){show(false)});\n document.addEventListener('keydown',function(e){if(e.key==='Escape')show(false)});\n if(mq.addEventListener)mq.addEventListener('change',apply);else if(mq.addListener)mq.addListener(apply);\n apply();\n})();\n</script></body></html>\n"
)
#!/usr/bin/env python3
"""
...
# code -> heading, and the storage key its filter selection is saved under.
# The keys are page-specific on purpose: index.html and 3rdspace.html are
# served from one origin, so a shared key would let one page's filters
# silently overwrite the other's.
VENUES = [
    ("M", "Mission E1", "blokTypesM"),
    ("B", "BLOK \u2014 Clapton &amp; Shoreditch", "blokTypes"),
]
LS_KEYS = {"a": "blokAvailOnly", "f": "blokHideFull", "w": "blokHideWorkHours",
           "lo": "blokLocations",
           "v": "blokFavs", "vv": "blokFavsSeed", "c": "blokCollapse",
           "bk": "blokBookedCol"}
...
SC = {"bookable":"#1f8a3c","full":"#c0392b","soon":"#b06a00","booked":"#1f7fd1",
      "closed":"#8a8a8f","unknown":"#6b6b76","open":"#41627d"}
...
GROUP = ('<div class="grp"><h3>%(label)s</h3>'
         '<div class="row fav" id="fav%(c)s"><label class="all">'
         '<input type="checkbox" id="allFav%(c)s" checked> all</label></div>'
         '<div class="row" id="cats%(c)s">'
         '<button type="button" class="col" id="col%(c)s" aria-expanded="true"></button>'
         '<label class="all"><input type="checkbox" id="allCat%(c)s" checked> all</label>'
         '</div></div>')

LOCS_HTML = ('<div class="grp"><h3>Locations</h3>'
             '<div class="row locs" id="locs"></div></div>\n')

def locs_html(locations):
    """A switch per club - only on pages that have more than one."""
    return LOCS_HTML if len(locations) > 1 else ""

def groups_html(venues):
    return "\n".join(GROUP % {"label": label, "c": code} for code, label, _ in venues)

def ls_object(venues, keys):
    """The page's localStorage keys, including one per venue for its filters."""
    o = dict(keys)
    o["t"] = {code: key for code, _, key in venues}
    return json.dumps(o, ensure_ascii=False, separators=(",", ":"))

def build(rows, template, today=None, venues=None, ls_keys=None,
          palette=None, sub_html=None, desc=None, studio_url=None, title=None,
          locations=None):
    ...
    order = {c: order_for(c) for c in codes}
    # The same class can run at more than one venue - Third Space teaches Just
    # Ride at all three clubs - so counts are kept per venue. Selection and
    # favourites are already keyed by venue, and a shared name deliberately
    # shares its colour.
    per_venue = {code: {c: sum(1 for r in rows if r[10] == code and r[4] == c)
                        for c in order[code]} for code in codes}
    pc = {}
    for c in order.get("B", []): pc[c] = CORE.get(c, GREY)
    for c in order.get("M", []): pc[c] = MISSION_CORE.get(c, GREY)
    for code in codes:
        for c in order[code]:
            pc.setdefault(c, palette.get(c, GREY) if palette else GREY)
    favs = [code + "|" + c for code in codes
            for c in order[code] if c in DEFAULT_FAVS]
    order_b, order_m = order.get("B", []), order.get("M", [])
    if desc is None:
        desc = descriptions(order_b + order_m)
    j = lambda o: json.dumps(o, ensure_ascii=False, separators=(",", ":"))
    if studio_url is None:
        studio_url = {name: "https://classpass.com/studios/" + slug
                      for name, slug, _ in STUDIOS}
    data = ("var D=%s,SC=%s,PC=%s,CATS=%s,VENUES=%s,ORDER=%s,DEFAULT_FAVS=%s,DESC=%s,STUDIO_URL=%s,LOCS=%s;" % (
        j(rows), j(SC), j(pc), j(per_venue),
    ...
    if sub_html is not None:
        ...
        html = (template.replace("@@DATA@@", data).replace("@@SUB@@", sub)
                .replace("@@GROUPS@@", groups_html(venues))
            .replace("@@LOCS@@", locs_html(locations))
                .replace("@@LOCS@@", locs_html(locations))
                .replace("@@LS@@", ls_object(venues, ls_keys)))
        if title:
            html = (html.replace("Class schedule \u2014 BLOK &amp; Mission E1", title)
                        .replace("<h1>Class schedule</h1>", "<h1>%s</h1>" % title))
        if "@@" in html:
            raise SystemExit("FATAL: unreplaced template token")
        return html, states, cats
    sub = ('<p class="sub">' + " &amp; ".join(link % (slug, name) for name, slug, _ in STUDIOS) +
           " \u00b7 %s \u2013 %s %s \u00b7 %d classes" + note +
           " \u00b7 auto-updated every 2 hours \u00b7 refreshed %s</p>") % (
...
def rebuild(out):
    ...
    try:
        ...
        i = html.index("var D=[")
        j = html.index("],SC=", i)
        rows = json.loads(html[i + 6:j + 1])
    except Exception as e:
        log("FATAL: cannot read the classes out of %s (%s) - leaving it alone" % (out, e))
        return 1
    if not rows:
        log("FATAL: no classes in %s - leaving it alone" % out)
        return 1
    ...
    log("  rebuilt %s from %d existing classes (%d bytes) states=%s"
        % (out, len(rows), len(new.encode()), states))
    return 0
