...
def build(rows, template, today=None, venues=None, ls_keys=None,
    ...
    # A category name must belong to exactly one venue: the filters, the
    # favourites and the colours are all keyed by name.
    for i, a in enumerate(codes):
        for b in codes[i + 1:]:
            clash = set(order[a]) & set(order[b])
            if clash:
                raise SystemExit("FATAL: category used by two venues (%s/%s): %s"
                                 % (a, b, sorted(clash)))
    pc = {}
    ...
    data = ("var D=%s,SC=%s,PC=%s,CATS=%s,VENUES=%s,ORDER=%s,DEFAULT_FAVS=%s,DESC=%s,STUDIO_URL=%s;" % (
        j(rows), j(SC), j(pc), j({c: cats[c] for code in codes for c in order[code]}),
