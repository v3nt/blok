...
def build(rows, template):
    ...
    data = ("var D=%s,SC=%s,PC=%s,CATS=%s,orderB=%s,orderM=%s,DEFAULT_FAVS=%s;" % (
        j(rows), j(SC), j(pc), j({c: cats[c] for c in order_b + order_m}),
        j(order_b), j(order_m), j(favs)))
